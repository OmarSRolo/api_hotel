...
class ImageManipulator
{
    ...
    public function resample_after_check($width, $height, $constrainProportions = true)
    {
        if (!is_resource($this->image)) {
            throw new RuntimeException('No image set');
        }
    ...
    public function watermark_image($target, $wtrmrk_file, $newcopy, $position = 'top')
    {
        $watermark = imagecreatefrompng($wtrmrk_file);
        imagealphablending($watermark, false);
        imagesavealpha($watermark, true);
        //var_dump(getimagesize($target)['mime']);
				$sExt	= getimagesize($target)[2];
        ...
        $dst_y = ($img_h) - ($wtrmrk_h) - 15; // For centering the watermark on any image
        }
